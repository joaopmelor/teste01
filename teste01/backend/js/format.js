...
function formatCPF(input) {
    ...
    cpf = cpf.replace(/(\d{3})(\d{1,2})$/, '$1-$2'); // Adicionar o hífen
...
function formatTelefone(input) {
    ...
    telefone = telefone.replace(/(\d{5})(\d)/, '$1-$2'); // Adicionar o hífen após o 5º dígito
...
function formatCEP(input) {
    ...
    cep = cep.replace(/(\d{5})(\d)/, '$1-$2'); // Adicionar o hífen
